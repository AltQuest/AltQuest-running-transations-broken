...
public class BlockEvents implements Listener {
	...
	public BlockEvents(AltQuest plugin) {
	...
	void onBlockCatchFire(BlockIgniteEvent event) {
		if(event.getCause().equals(IgniteCause.FLINT_AND_STEEL)) {
			if(event.getPlayer() != null) {
				if (!altQuest.canBuild(event.getBlock().getLocation(), event.getPlayer())) {
    ...
    void onBlockBreak(BlockBreakEvent event) {
    	...
    	if(event.getBlock().getType().equals(Material.BEDROCK)) {
    		altQuest.error(event.getPlayer(), "Removing bedrock is not allowed!");
    		event.setCancelled(true);
    	// If player is in a no-build zone, cancel the event
    	} else if (!altQuest.canBuild(event.getBlock().getLocation(), event.getPlayer())) {
			event.setCancelled(true);
			altQuest.error(event.getPlayer(), "You may not break blocks here!");
        } else {
	...
	void onBlockPlace(BlockPlaceEvent event) {
		...
		if (!altQuest.canBuild(event.getBlock().getLocation(), event.getPlayer())) {
			event.setCancelled(true);
			altQuest.error(event.getPlayer(), "You may not place blocks here!");
		} else if(event.getBlock().getType().equals(Material.BEDROCK)) {
			altQuest.error(event.getPlayer(), "Placing bedrock is not allowed!");
			event.setCancelled(true);
		} else {
			event.setCancelled(false);
		}
    ...
    void onPistonExtends(BlockPistonExtendEvent event) {
        ...
        if (!blocks.isEmpty()) {
            ...
            if ((owner2 = AltQuest.REDIS.get("chunk" + blockChunk.getX() + "," + blockChunk.getZ() + "owner")) != null) {
                if ((owner1 = AltQuest.REDIS.get("chunk" + pistonChunk.getX() + "," + pistonChunk.getZ() + "owner")) != null) {
