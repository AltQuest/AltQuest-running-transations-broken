...
Material.WHITE_SHULKER_BOX, Material.YELLOW_SHULKER_BOX);
    ...
    public void onPlayerJoin(PlayerJoinEvent event) throws IOException, org.json.simple.parser.ParseException, ParseException, JSONException {
        ...
        if (altQuest.isModerator(player)) {
            ...
            player.sendMessage(ChatColor.YELLOW + "The world BTC wallet balance is: " + altQuest.wallet.final_balance() / 100 + " bits");
            player.sendMessage(ChatColor.BLUE + "" + ChatColor.UNDERLINE + "blockchain.info/address/" + altQuest.wallet.address);
player.sendMessage(ChatColor.YELLOW + "The world DOGE wallet balance is: " + altQuest.DOGE_wallet.final_balance() + " DOGE");
  ...
  public void onPlayerMove(PlayerMoveEvent event) throws ParseException, org.json.simple.parser.ParseException, IOException {
        ...
        if (event.getItem() != null) {
            final Player player=event.getPlayer();
                ...
                if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
                    ...
                    player.sendMessage(ChatColor.GREEN+"Teleporting to SatoshiSquare...");
                    player.setMetadata("teleporting", new FixedMetadataValue(altQuest, true));
                    World world=Bukkit.getWorld("world");
    ...
    void onEntityDeath(EntityDeathEvent e) throws IOException, ParseException, org.json.simple.parser.ParseException {
        ...
        if (entity instanceof Monster) {
            ...
            if (e.getEntity().getLastDamageCause() instanceof EntityDamageByEntityEvent) {
                ...
                if (damage.getDamager() instanceof Player && level >= 1) {
                    ...
                    final User user = new User(player);
		    final double lootmx = AltQuest.rand(1,4);
                    final int money = (int)(AltQuest.LAND_PRICE()/lootmx);
			...
			if (whatLoot>=9250){                    
                    ...
                    if(altQuest.wallet.final_balance()>money) {

			if (money>=7000&&money<=500000){
			final BukkitScheduler scheduler = Bukkit.getServer().getScheduler();
                        ...
                        scheduler.runTaskAsynchronously(altQuest, new Runnable() {
                            ...
                            public void run() {
				try {
				if (altQuest.wallet.blockcypher_microtransaction(money, userWallet.address)){
				System.out.println("[loot]"+whatLoot+" "+player.getDisplayName()+": Bitcoin "+money);
                        ...
                        scheduler.runTaskAsynchronously(altQuest, new Runnable() {
                            ...
                            public void run() {
                                try {
					if (altQuest.wallet.create_blockcypher_transaction(money,  userWallet.address)) {
                                        System.out.println("[loot]"+whatLoot+" "+player.getDisplayName()+": Bitcoin "+money);
                                        player.sendMessage(ChatColor.GREEN + "You got " + ChatColor.BOLD + money / 100 + ChatColor.GREEN + " bits of loot!");
                                        // player.playSound(player.getLocation(), Sound.LEVEL_UP, 20, 1);
                                        if (altQuest.messageBuilder != null) {
		...
		//start if DOGE
			...
			long TDLoot= (long)altQuest.getSwapValue(money*0.00000001, "btc", "doge");		
			long Doge_Loot =((long) (TDLoot/0.00000001));
			System.out.println("DOGE LOOT is:"+(Doge_Loot*0.00000001));
			System.out.println("DOGE WALLET Balance is:"+altQuest.DOGE_wallet.balance());
                    if(altQuest.DOGE_wallet.balance()>=(Doge_Loot*0.00000001)) {
                        ...
                        scheduler.runTaskAsynchronously(altQuest, new Runnable() {
                            @Override
                            public void run() {
				System.out.println("Trying to produce a DOGE TX. . .");
                                try {
                                    if (altQuest.DOGE_wallet.create_blockcypher_transaction((Doge_Loot*0.00000001), user.DOGE_wallet.address)) {
